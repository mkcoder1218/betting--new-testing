...
const BetSlipTable = ({ type, data }: ActionType) => {
  ...
  return (
    <>
      {!resultVisible && <div className="right-flex pl-3 pr-3 flex-grow border-l-2 border-slate-200 ml-6">
        ...
        <div className="slip-container mt-3 relative overflow-x-auto">
          <table className="w-full text-sm text-left rtl:text-right">
            <thead className="text-xs border-b-2 border-t-2 border-slate-300 text-gray-700 uppercase bg-gray-50">
              <tr>
                {type === "redeem" && (
                  <th colSpan={1} scope="col" className="px-6 col-span-4 py-3">
                    Actions
                  </th>
                )}
                <th scope="col" className="px-3 py-3">
              ...
              {data.Tickets?.map((item) => {
                return (
                  <tr key={item.id} className={`${item.win > 0 ? 'bg-orange-300 text-white' : 'bg-white'} border-b`}>
                    ...
                    <td className="px-3 py-2">
                      {item.win && item.win > 0 ? item.win.toFixed(2) : 0.0} Br
                    </td>
                  </tr>
                );
              })}
      ...
      {resultVisible &&
        <div className="right-flex pl-3 pr-3 flex-grow border-l-2 border-slate-200 ml-6">
          <div className="result-header flex justify-between">
            ...
            <div>
              <button onClick={() => toggleResult(false)} className="flex p-2 border-2 border-gray-400 hover:bg-slate-200 transition-all items-center rounded-md"><IoChevronBackOutline size={24} /> Back To List</button>
            </div>
          </div>
