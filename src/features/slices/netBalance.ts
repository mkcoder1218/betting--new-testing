...
export const getNetBalance = (shopId: string | undefined) => async (dispatch: (arg0: { payload: BalanceState; type: "netbalance/addNetBalance"; }) => void) => {
    dispatch(addNetBalance({ loading: true, error: null, message: null, data: null }));

    try {
        const netBalanceRes: ApiResponse = (await axiosInstance.get(`ticket/balance/${shopId}`)).data;

        if (netBalanceRes.message === "success") {
            dispatch(addNetBalance({ loading: false, error: null, message: netBalanceRes.message, data: netBalanceRes.data }));
        } else {
