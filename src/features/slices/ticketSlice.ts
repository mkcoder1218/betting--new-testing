...
export const printSelectedTickets = async (req: any) => {
    try {
        ...
        if (printSelectedResponse.status === 200 || printSelectedResponse.status === 201) {
          const printSelectedData = await axios.post("http://0.0.0.0:6000/printEvent", printSelectedResponse.data.data)
        }
    } catch (err) {
        console.log(err);
    }
...
export const isPrinterUp = async () => {
  try {
    const printerStatus = await axios.get("http://0.0.0.0:6000/check");
