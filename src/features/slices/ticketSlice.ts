...
export const printSelectedTickets = async (req: any) => {
    try {
        ...
        if (printSelectedResponse.status === 200 || printSelectedResponse.status === 201) {
          const printSelectedData = await axios.post("http://127.0.0.1:5000/printEvent", printSelectedResponse.data.data)
        }
    } catch (err) {
        console.log(err);
    }
...
export const isPrinterUp = async () => {
  try {
    const printerStatus = await axios.get("http://127.0.0.1:5000/check");
